...
| 2026-07-19 | (Structural refactor) | §3, §4.1, §11 | Podium (top 3) + Rankings List (4th+) moved from `/dashboard/leaderboard` onto `/dashboard` itself (below the Metric Graph, above the Recent Activities feed), now driven entirely by the dashboard's own `activeMetric`/`activeRange` URL-param state — no separate filters. `/dashboard/leaderboard` route removed; repurposed as `/dashboard/challenges`, which now hosts the three-tab Challenges module (previously mounted on `/dashboard`). Nav labels/icons/hrefs updated (`Sidebar.tsx`, `MobileBottomNav.tsx`). Shared ranking aggregation extracted to `lib/queries.ts`'s `getLeaderboardEntries()` (same scoring algorithm, unchanged). |
| 2026-07-22 | (Cache Reliability) | §7.1 (new) | Updated all mutation Server Actions to invoke `revalidatePath('/', 'layout')` instead of exact-path revalidations, eliminating stale ghost data on child/sister routes (`/dashboard/gang`, `/profile/[userId]`, etc.). |
...
All cache invalidations across Server Actions enforce layout-wide revalidation (`revalidatePath('/', 'layout')`) to ensure graph, podium, rankings, and all sibling/child components (`/dashboard`, `/dashboard/gang`, `/profile/[userId]`, etc.) refresh simultaneously without stale ghost data.

| Server Action | Path | Reason |
|---|---|---|
| `ingestActivity` | `('/', 'layout')` | New `metric_logs` row → feed + chart + gang + profile |
| `logDirectActivity` / `logActivityManual` | `('/', 'layout')` | New `metric_logs` row → all views |
| `processVerificationVote` (approve or reject) | `('/', 'layout')` | Status transition + potential row deletion |
| `deleteActivityAction` | `('/', 'layout')` | Log removal → purged from all views |
...
| `createMetricDefinition` / `adminUpdateMetricDefinition` / `adminDeleteMetricDefinition` / `adminToggleMetricHidden` | `('/', 'layout')` | Metric definition list changes |
| `adminUpdatePersistentMood` | `('/', 'layout')` | Bot state changes |
| `connectWearableAction` / `disconnectWearableAction` | `('/', 'layout')` | Wearable connection state |
