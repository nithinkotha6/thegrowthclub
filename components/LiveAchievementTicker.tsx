...
export default async function LiveAchievementTicker({ groupId }: { groupId: string }) {
  ...
  return (
    ...
    >
      ...
      <div className="z-10 bg-slate-900 pl-3 pr-2 py-1 flex-shrink-0 w-[28%] max-w-[135px] md:w-auto md:max-w-none flex items-center gap-1.5 font-mono font-black tracking-wider text-[#CEFF00] uppercase text-[9px] md:text-xs border-r border-slate-800">
      ...
      <div className="flex-grow overflow-hidden flex items-center">
        <div className="animate-ticker-marquee" style={{ willChange: 'transform' }}>
          {doubled.map((sentence, i) => (
            <span key={i} className="inline-flex items-center gap-2 font-mono font-medium tracking-wide text-white text-sm md:text-base mr-16">
              <span>{sentence}</span>
            </span>
          ))}
        </div>
